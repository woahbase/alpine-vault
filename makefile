...
VERSION   ?= 1.14.10#$(call get_gh_version,hashicorp/vault)
...
BUILDFLAGS ?= \
	...
	--build-arg SRCARCH=$(call get_src_arch) \
...
OTHERFLAGS   := \
	...
	--cap-add IPC_LOCK \
	# \
	# -e VAULT_ARGS='-config=/vault/config/server.hcl' \
	# -e VAULT_ARGS='-dev -dev-listen-address=0.0.0.0:8200 -dev-root-token-id=roottoken' \
